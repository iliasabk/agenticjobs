...
test('a numeric character reference past the last code point does not kill the import', () => {
  ...
  // import. A page can carry them innocently - they become U+FFFD.
  ...
  assert.equal(job.via, 'page');
  assert.ok(job.title.includes('\uFFFD'), job.title);
  assert.ok(job.description.includes('\uFFFD'), job.description);
});
